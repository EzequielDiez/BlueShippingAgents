...
const Navbar = ({ isInitialAnimation }) => {
    ...
    return (
        <nav className={`fixed top-0 left-0 right-0 ${isInitialAnimation ? 'h-[50vh] bg-white' : 'h-24 bg-[#1D4F87]'} px-4 text-white z-50 transition-all duration-2500 ease-in-out ${isVisible ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'}`}>
            <div className="max-w-7xl mx-auto relative flex justify-between items-center h-full">
            ...
            {!isInitialAnimation && (
                <div className={`language-toggle absolute right-4 top-12 ${isFirstLoad ? 'transition-all duration-1000 delay-1000' : ''} ${showLinks ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                    <button
                        onClick={toggleLanguage}
                        className="relative inline-flex items-center h-7 w-20 rounded-full bg-transparent border border-white transition-all duration-300 hover:bg-white/20 hover:shadow-lg cursor-pointer"
                    >
